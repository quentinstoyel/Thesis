...
As the various technological restraints of electron microscopes improve, the limiting factor in probing materials shifts increasingly towards dosage limits.  The various electron beam sample interactions that occur can damage materials in a multitude of ways.  These can include, charging, heating, sputtering (mass loss) and crystal deformation.  The resulting damage impacts results, decreases signal, and limits sample use.  Mass loss and crystal deformation are most relevant to EELS as they reduce signal and disrupt the local symmetry that is measured in the ELNES.  These effects are caused largely by radiolysis and knock-on damage.  

\subsubsection{Knock On Damage}
Knock on damage results from a beam electron transferring enough energy to a nucleus in the sample to displace it from its lattice site.  The effect arises from high angle elastic scattering as the energy transferred for this interaction is defined by: 

\begin{equation}
E = E_{\mathrm{max}} \mathrm{sin}^2 (\theta/2)
\end{equation}

with $E_{\mathrm{max}}$ depends on the beam energy and the atomic mass, as:
...
\caption{Plot relating $E_{\mathrm{max}}$ to beam energy for a variety of elements, as calculated in Eq. \ref{E_max_eq}.  Taken from Egerton \cite{egerton_basic_2010}.}
\label{E_max_plot}
\end{figure}
...
One means of reducing knock on damage is thus to perform EELS at lower operating voltages, which reduces $E_{\mathrm{max}}$. Lower beam energy also increases the cross section of elastic scattering for lithium, according to the Mott cross section.  For heavier elements, it is possible to drop $E_{\mathrm{max}}$ below a threshold energy and eliminate knock on damage, however even at 30keV, $E_{\mathrm{max}}$ is great enough to damage lithium containing specimens.  

\subsubsection{Radiolysis}
Radiolysis refers to the breaking of atomic bonds, ionizing atoms and resulting in diffusion and loss of crystal structure.  Organic materials with low electron conductivity are highly susceptible to radiolysis, especially as it can remove highly mobile hydrogen atoms from hydrocarbons.  Metallic samples are less affected as the free electrons can screen the resulting charge discrepancies long enough for atoms to reform the bonds.   Lithium materials are also vulnerable to radiolysis due to their non conductive nature and lithium's high mobility.  Particularly affected are lithium halogens such as lithium fluoride (LiF) \cite{egerton_electron_1987}. In these materials, radiolysis from F-centres and H-centres, separating the elements and allowing the fluorine to diffuse from the material \cite{egerton_electron_1987}.  Radiolysis is dependent on temperature and using a cryogenic sample holder is one option of limiting, or at least slowing down this effect by impeding diffusion.  
...
Lithium materials present a number of challenges to EELS analysis.  Lithium ion battery materials are in general semiconductors with band gaps of varying sizes. Coupled with lithium's highly mobile nature, lithium materials are highly sensitive to both knock on and radiolysis damage.  Lithium's high mobility also makes it vulnerable to knock on damage.   Lithium is particularly vulnerable to this interaction because it requires only a small amount of energy ($ \sim 0.2-3$eV) to displace it through a crystal \cite{kang_factors_2006}.  Lithium's sensitivity to the beam make EELS's short acquisition times, on the order of seconds, well suited for its analysis. Recent developments enabling EELS at 30keV have made it applicable to a new range of lithium materials.\\  
